...
func runInternetScan(limit int, users, passwords []string, opts *Options, outFile *os.File) error {
	...
	// Create scanner
	s := scanner.NewScanner(opts.Verbose, outFile, time.Duration(opts.Timeout)*time.Second, int32(limit))
...
func runTargetScan(targets, users, passwords []string, opts *Options, outFile *os.File) error {
	...
	// Create scanner
	s := scanner.NewScanner(opts.Verbose, outFile, time.Duration(opts.Timeout)*time.Second, 0)
